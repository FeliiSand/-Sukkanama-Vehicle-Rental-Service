...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($user_result->num_rows == 1) {
        ...
        if ($row['user_type'] == 'customer') {
            ...
            // Query to fetch vehicle details added by suppliers
            $vehicle_sql = "SELECT * FROM Vehicle WHERE supplier_id IN (SELECT user_id FROM User WHERE user_type = 'supplier')";
            $vehicle_result = $conn->query($vehicle_sql);

            if ($vehicle_result->num_rows > 0) {
                // Fetch vehicle details and store in array
                while ($vehicle_row = $vehicle_result->fetch_assoc()) {
                    $vehicle_details[] = $vehicle_row;
                }
            }
            ...
            // User is not a customer, display error message
            $login_error = "Only customers are allowed to login.";
        }
    } else {
...
<head>
    ...
    <title> Sukkanama Customer Login</title>
...
<div class="container">
    ...
    <div class="register-link">
        <p>Not registered? <a href="signup_customer.php">Register here</a></p>
    </div>
</div>
